...
#define irMin    4 // The IR threshold for presence of an obstacle in ADC value
...
//Homing loop
  ...
  if (obstacle == true){
    ...
    digitalWrite(redLED, LOW);//turn on yellow LED
...
//    }
    ...
    (IrF == true && IrL == true && IrR == false && IrB == false )||
    (IrF == false && IrL == true && IrR == false && IrB == true)||
    ...
    (IrF == false && IrL == true && IrR == true && IrB == false )||
    (IrF == false && IrL == true && IrR == true && IrB == true)){
      state = fwd;
    }else if (IrL == true && IrR == true && SonarL == true && SonarR == true && IrB == true){
      state = botStop;
    }
...
void updateState2(){
  if (IrR==true){
    if(inirR > irThresh){
    digitalWrite(redLED, LOW);//turn off red LED
    digitalWrite(ylwLED, HIGH);//turn on yellow 
      state = goleft;
    }else if(inirR < irMin){
      state = goright;
    digitalWrite(redLED, HIGH);//turn off red LED
    digitalWrite(ylwLED, LOW);//turn on yellow 
    }else{
      state = fwd;
          digitalWrite(redLED, LOW);//turn off red LED
    digitalWrite(ylwLED, LOW);//turn on yellow 
...
void BangBang(){
  if(state == goleft){
    if (turnedleft == false){
      spin(-20);
      forward(one_rotation*0.5, robot_spd);
      turnedleft = true;
    ...
    if(turnedleft == true){
      turnedleft = false;
      spin(15);
      forward(one_rotation*1, robot_spd);
    }else if(turnedright == true){
      turnedright = false;
      spin(-15);
      forward(one_rotation*1, robot_spd);
