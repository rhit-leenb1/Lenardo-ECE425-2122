...
if (InputLength>0){
  if (Input.charAt(0)=="L"){
    if(IrL == true && IrR == true && IrF == false){
      forward(qrtr_rot*1,robot_spd);//adjust the constant to make the robot move one block);
      stop();
    }else if((IrL == true && IrR == false && IrF == false)||(IrL == true && IrR == false && IrF == true)){
      delay(500);
      spin(-90);
      forward(qrtr_rot*1,robot_spd);//adjust the constant to make the robot move one block);
    }else if((IrL == false && IrR == true && IrF == false)||(IrL == false && IrR == true && IrF == true)){
      delay(500);
      spin(90);
      forward(qrtr_rot*1,robot_spd);//adjust the constant to make the robot move one block);
      Input.remove(0);
    ...
    if(IrL == true && IrR == true && IrF == false){
      ...
      spin(90);
      forward(qrtr_rot*1,robot_spd);//adjust the constant to make the robot move one block);
    }else if((IrL == false && IrR == true && IrF == true)||(IrL == true && IrR == false && IrF == true)){
      stop();
      Input.remove(0);
      InputLength = Input.length();
    }
